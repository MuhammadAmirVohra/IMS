   ...
   const arr = ['Requested By','Email','Date Requested','Item Name','Quantity','Duration']
    ...
    const ModalContent = ()=>{
       const [comment , setcomment] = useState("")
        ...
        return(
            <Modal show = {showModal} onHide ={() => { let f = setInterval(()=>{fetch()}, 3000); setinternal_id(f); setShow(false)}}>
     ...
     <Container>
        ...
        <tbody>
          {allrequest.map((request, index) =>{
            return(
            <>
            <tr onClick = {() => {  clearInterval(internal_id); SetInfo(request); setShow(true);} }>
                <td>{index+1}</td>
