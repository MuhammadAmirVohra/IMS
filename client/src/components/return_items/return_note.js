...
const ReturnNote = () => {
  ...
  useEffect(() => {
    axios.get(API_URL + "/storeitemreturn").then((res) => {
  ...
  const handleInputChange = (e, index) => {
    ...
    list[index].index = allitems
      .map((e) => e.Item_ID.Item_Name)
      .indexOf(e.target.value);
    list[index].item_id =
      allitems[
        allitems.map((e) => e.Item_ID.Item_Name).indexOf(e.target.value)
      ].Item_ID._id;
    list[index].item_number =
      allitems[
        allitems.map((e) => e.Item_ID.Item_Name).indexOf(e.target.value)
      ].Item_ID.Item_ID;
  ...
  const Submit = (e) => {
    ...
    if (validation()) alert("Values Missing!");
    else {
      axios
        .post(API_URL + "/returnitems", {
          items: items,
          department: department,
          date: date,
        ...
        .then((res) => {
          if (res.data.code === 200) {
            ...
            setTimeout(() => {
              window.flash("Item Successfully Returned ");
            }, 500);
          }
        });
    }
    ...
    <>
      ...
      <Container className="Container1 justify-content-center">
        <Form onSubmit={Submit} className="w-100 FormStyle">
          <h1 className="Heading">Return Note</h1>
          ...
          <Form.Group className="FormGroup">
            ...
            <Form.Control
              ...
              onChange={(e) => {
                ...
                setreqid(
                  requests[requests.map((e) => e._id).indexOf(e.target.value)]
                    .Order_ID
                );
              }}
            >
              ...
              {requests.map((request, index) => {
                return (
                  <option value={request._id} className="new-line">
                    {request.Order_ID} - {request.Item}
                  </option>
                );
              })}
            </Form.Control>
          ...
          <h4 className="w-100 CenterLabel">
            Items
            <Button
              ...
              <Form.Group className="FormGroup w-100">
                ...
                >
                  ...
                  {allitems.map((item, index) => {
                    return (
                      <option value={item.Item_ID.Item_Name}>
                ...
                <Form.Control
                  ...
                  type="Number"
                  // Min="1"
                  // Max={items[i].index !== undefined ? allitems[items[i].index].Issued : 1}
                  name="quantity"
                  placeholder="Quantity"
                  // placeholder={items[i].index !== undefined ? "Min : 1 - Max : " + allitems[items[i].index].Issued : "Quantity"}
                  value={x.quantity}
          ...
          <Button
            className="btn-lg float-right  ml-2 btn-success"
            onClick={PDF}
          >
          ...
          <Button
            className="btn-lg float-right Btn"
            type="submit"
            onClick={Submit}
          >
            Return
